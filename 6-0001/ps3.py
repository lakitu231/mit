...
def is_valid_word(word, hand, word_list):
    ...
    if word.find('*') == -1:
        ...
        track_hand = hand.copy()
        for letter in word.lower():
            if letter not in track_hand.keys():
                return False
            else:
                if track_hand[letter] == 1:
                    del(track_hand[letter])
                else:
                    track_hand[letter] -= 1
        ...
        for wild_char in VOWELS:
            wild_word = word.replace('*', wild_char)
            
            wild_hand = hand.copy()
            wild_hand[wild_char] = wild_hand.get(wild_char, 0) + 1
            
            if is_valid_word(wild_word, wild_hand, word_list):
                return True
        return False
...
def substitute_hand(hand, letter):
    ...
    for letter in set(sub_hand.keys()):
        pool.replace(letter, '')
    
    if letter in sub_hand.keys():
        sub_hand[random.choice(pool)] = sub_hand[letter]
        del(sub_hand[letter])
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while series > 0:
        ...
        if not first and not replayed:
            replay_prompt = input("Would you like to replay the hand? ").lower()
            if replay_prompt == 'yes':
                replayed = True
                replaying = True
        ...
        if not substituted and not replaying:
